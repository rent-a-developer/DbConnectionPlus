# Claude Code PostToolUse hook: format the C# file this edit touched, with CSharpier.
#
...
function Resolve-EligibleFile
{
    <#
        ...
        Rejected: anything that is not a .cs file, anything that no longer exists (a delete, or a move's old
        name), anything under bin/ or obj/, and anything that resolves outside the repository root. The last
        one is what stops a payload naming ../../etc/something, and it is checked AFTER resolution, so a
        symbolic link or junction that points out of the tree is rejected too - Resolve-Path follows it and
        the result no longer starts with the root.
    #>
    param([Parameter(Mandatory)] [String] $Root, [String] $Path)

    if ([String]::IsNullOrWhiteSpace($Path)) { return $null }
    if ([System.IO.Path]::GetExtension($Path) -ne '.cs') { return $null }
...
try
{
    ...
    $mutexName = 'Global\dbconnectionplus-tidy-' +
    ...
    if ($exitCode -ne 0)
    {
        Write-Output "CSharpier failed on $file. The build treats formatting as an error, so fix this before building:`n$output"
    }
}
catch
{
