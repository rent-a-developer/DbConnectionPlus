...
function Resolve-EligibleFile
{
    <#
        ...
        Rejected: anything that is not a .cs file, anything that no longer exists, anything under bin/ or
        obj/, and anything that resolves outside the repository root. The last check happens AFTER
        resolution, so a symbolic link or junction pointing out of the tree is rejected too.
    #>
