...
.DESCRIPTION
    ...
    entry, and could be committed by accident.

    Keep-a-Changelog link reference definitions - the `[4.1.0]: https://...` lines that some changelogs
    collect at the end of the file, and that a section can also carry - are dropped from the extracted notes.
    They resolve against the changelog, not against a GitHub release page, where they render as nothing or as
    a broken link.

    On failure the script emits a GitHub Actions ::error annotation and exits non-zero.

.PARAMETER Version
    The version to extract, without the leading "v" - for example 4.1.0.
...
.EXAMPLE
    pwsh -File scripts/extract-release-notes.ps1 -Version 4.1.0
#>
...
# A link reference definition: `[4.1.0]: https://github.com/...`, at the start of a line.
$linkDefinition = '^\[[^\]]+\]:\s'
