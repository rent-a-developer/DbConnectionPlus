...
.DESCRIPTION
    ...
    disposable copy before it runs the tools, which stores every file as LF whatever was on disk, and
    CSharpier rewriting CRLF to LF is then a change `git diff` reports as nothing at all.
...
Write-Output ''
Write-Output 'Wrong on disk only (i/lf w/crlf): git checkout-index --force --all rewrites the working tree'
Write-Output 'from the index. Nothing is lost - git already reports these files as unmodified, which is'
Write-Output 'exactly why nothing else notices them.'
