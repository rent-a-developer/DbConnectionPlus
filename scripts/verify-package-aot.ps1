...
.DESCRIPTION
    ...
    broken annotation chain. See the "Native AOT and Trimming" section of DESIGN-DECISIONS.md.
...
.PARAMETER PackageVersion
    The version of the packages to consume. Defaults to the <Version> in src/Directory.Build.props, which is
    what `dotnet pack` produces.
...
if (-not $PackageVersion)
{
    ...
    $sharedProperties = Join-Path $repositoryRoot 'src/Directory.Build.props'
...
if ($Pack)
{
    ...
    # NuGet resolves by version, not by content. Without this the consumer would happily restore the previous
    # build of the same version number out of the isolated cache and the publish would prove nothing.
...
$expectedPackage = Join-Path $packageDirectory "DbConnectionPlus.$PackageVersion.nupkg"

if (-not (Test-Path $expectedPackage))
{
    Write-Host "FAILED. $expectedPackage does not exist." -ForegroundColor Red
    Write-Host ''
...
# See the "No consumer-facing diagnostics" section of DESIGN-DECISIONS.md for the full argument.
#
