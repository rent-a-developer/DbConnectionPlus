...
public static partial class DbConnectionExtensions
{
    ...
    /// into an instance of the type <typeparamref name="T" /> or the default of <typeparamref name="T" /> if the
    /// statement did not return any rows.
    /// </summary>
    /// <typeparam name="T">
    /// The type of object to materialize the first row of the result set to. See remarks for details.
    /// </typeparam>
    ...
    /// <returns>
    /// An instance of the type <typeparamref name="T" /> containing the data of the first row of the result set
    /// returned by the statement or the default of <typeparamref name="T" /> if the statement did not return any rows.
    /// </returns>
            ...
            try
            {
                ...
                using (reader)
                {
                    ...
                    if (isTBuiltInTypeOrEnumType)
                    {
                        ...
                        return ConvertValueForQuery<T>(value);
                    }

                    if (isTValueTupleType)
                    {
                        return valueTupleMaterializer(reader);
                    }
    ...
    /// by the statement into an instance of the type <typeparamref name="T" /> or the default of
    /// <typeparamref name="T" /> if the statement did not return any rows.
    /// </summary>
    /// <typeparam name="T">
    /// The type of object to materialize the first row of the result set to. See remarks for details.
    /// </typeparam>
    ...
    /// the data of the first row of the result set returned by the statement or the default of
    /// <typeparamref name="T" /> if the statement did not return any rows.
    /// </returns>
    /// <exception cref="ArgumentNullException"><paramref name="connection" /> is <see langword="null" />.</exception>
    /// <exception cref="ArgumentException">
