...
public static partial class DbConnectionExtensions
{
    ...
    /// of instances of the type <typeparamref name="T" />.
    /// </summary>
    /// <typeparam name="T">The type of objects to materialize the result set to. See remarks for details.</typeparam>
    ...
    /// <returns>
    /// A sequence of instances of the type <typeparamref name="T" /> containing the data of the result set returned by
    /// the statement.
    /// </returns>
    ...
    /// </code>
    /// </example>
    public static IEnumerable<T> Query<[DynamicallyAccessedMembers(EntityHelper.QueryResultMemberTypes)] T>(
            ...
            using (reader)
            {
                ...
                while (true)
                {
                    ...
                    if (isTBuiltInTypeOrEnumType)
                    {
                        ...
                        {
                            yield return alreadyTargetTypeValue;
                        }
                        else
                        {
                            yield return ConvertValueForQuery<T>(value);
                        }
                    }
                    else if (isTValueTupleType)
                    {
                        yield return valueTupleMaterializer(reader);
                    }
                    else if (isTEntityType)
                    {
                        yield return entityMaterializer(reader);
                    }
                }
            }
        }
    ...
    /// into a sequence of instances of the type <typeparamref name="T" />.
    /// </summary>
    /// <typeparam name="T">The type of objects to materialize the result set to. See remarks for details.</typeparam>
    /// <param name="connection">The database connection to use to execute the statement.</param>
    /// <param name="statement">The SQL statement to execute.</param>
    ...
    /// <param name="cancellationToken">A token that can be used to cancel the operation.</param>
    /// <returns>
    /// An asynchronous sequence of instances of the type <typeparamref name="T" /> containing the data of the result
    ...
    /// If the conversion fails this method throws exceptions with messages matching the context of the Query methods.
    /// </summary>
