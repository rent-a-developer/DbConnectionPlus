...
public sealed class DbCommandBuilderTests_MySql : DbCommandBuilderTests<MySqlTestDatabaseProvider>;

public sealed class DbCommandBuilderTests_Oracle : DbCommandBuilderTests<OracleTestDatabaseProvider>;

public sealed class DbCommandBuilderTests_PostgreSql : DbCommandBuilderTests<PostgreSqlTestDatabaseProvider>;

public sealed class DbCommandBuilderTests_Sqlite : DbCommandBuilderTests<SqliteTestDatabaseProvider>;

public sealed class DbCommandBuilderTests_SqlServer : DbCommandBuilderTests<SqlServerTestDatabaseProvider>;
    ...
    where TTestDatabaseProvider : ITestDatabaseProvider, new()
{
    ...
    public async Task BuildDbCommand_ShouldCreateTemporaryTables(bool useAsyncApi)
    {
        ...
        (
            await this
                .Connection.QueryAsync<long>($"SELECT {Q("Value")} FROM {QT(temporaryTables[0].Name)}")
        ...
        (
            await this
                .Connection.QueryAsync<Entity>($"SELECT * FROM {QT(temporaryTables[1].Name)}")
                .ToListAsync(TestContext.Current.CancellationToken)
        )
            .Should()
    ...
    public async Task BuildDbCommand_ShouldReturnDisposerForCommandWhichDisposesTemporaryTables(bool useAsyncApi)
    {
        ...
        InterpolatedSqlStatement statement = $"""
            ...
            ON         Entities.Id = Ids.Value
            """;
        var (_, commandDisposer) = await CallApi(useAsyncApi, statement, this.DatabaseAdapter, this.Connection);
        ...
        this.ExistsTemporaryTableInDb(table1Name).Should().BeTrue();

        this.ExistsTemporaryTableInDb(table2Name).Should().BeTrue();

        await commandDisposer.DisposeAsync();

        this.ExistsTemporaryTableInDb(table1Name).Should().BeFalse();

        this.ExistsTemporaryTableInDb(table2Name).Should().BeFalse();
    }
    ...
    public async Task BuildDbCommand_ShouldSetCommandTimeout(bool useAsyncApi)
    {
        ...
        var (command, _) = await CallApi(useAsyncApi, "SELECT 1", this.DatabaseAdapter, this.Connection, null, timeout);
    ...
    {
        var (command, _) = await CallApi(useAsyncApi, "SELECT 1", this.DatabaseAdapter, this.Connection);

        command.Connection.Should().BeSameAs(this.Connection);
    }
    ...
    public async Task BuildDbCommand_ShouldSetParameters(bool useAsyncApi)
    {
        ...
        command.Parameters["EntityId"].Value.Should().Be(entityId);

        command.Parameters["DateTimeValue"].Value.Should().Be(dateTimeValue);

        command.Parameters["StringValue"].Value.Should().Be(stringValue);
    }
    ...
    public async Task BuildDbCommand_ShouldSetTransaction(bool useAsyncApi)
    {
        ...
        var (command, _) = await CallApi(useAsyncApi, "SELECT 1", this.DatabaseAdapter, this.Connection, transaction);
    ...
    public async Task BuildDbCommand_ShouldUseCancellationToken(bool useAsyncApi)
    {
        ...
        var exception = (
            await Invoking(() => command.ExecuteNonQueryAsync(cancellationToken)).Should().ThrowAsync<Exception>()
        ).Subject.First();
