    ...
    where TTestDatabaseProvider : ITestDatabaseProvider, new()
{
    ...
    [InlineData(true)]
    public async Task
        QuerySingle_ComplexObjectsTemporaryTable_ShouldDropTemporaryTableAfterExecution(bool useAsyncApi)
    {
    ...
    [InlineData(true)]
    public async Task
        QuerySingle_EntityType_CharEntityProperty_ColumnContainsStringWithLengthNotOne_ShouldThrow(bool useAsyncApi)
    {
        if (this.TestDatabaseProvider is not OracleTestDatabaseProvider)
        {
            // Oracle doesn't allow to return an empty string, because it treats empty strings as NULLs.
    ...
    [InlineData(false)]
    [InlineData(true)]
    public async Task QuerySingle_EntityType_CompatiblePrivateConstructor_ShouldUsePrivateConstructor(
        bool useAsyncApi
    )
    {
        var entity = this.CreateEntityInDb<Entity>();
    ...
    [InlineData(true)]
    public async Task
        QuerySingle_ScalarValuesTemporaryTable_ShouldDropTemporaryTableAfterExecution(bool useAsyncApi)
    {
        Assert.SkipUnless(this.DatabaseAdapter.SupportsTemporaryTables(this.Connection), "");
    ...
    [InlineData(false)]
    [InlineData(true)]
    public Task QuerySingle_ValueTupleType_NonNullableValueTupleField_ColumnContainsNull_ShouldThrow(
        bool useAsyncApi
    )
    {
        this.Connection.ExecuteNonQuery(
