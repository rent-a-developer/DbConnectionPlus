...
public class DbConnectionExtensions_ExecuteNonQueryTests()
    ...
    )
{
    ...
    {
        ArgumentNullGuardVerifier.Verify(() => this.MockDbConnection.ExecuteNonQuery("DELETE FROM Entity"));

        ArgumentNullGuardVerifier.Verify(() => this.MockDbConnection.ExecuteNonQueryAsync("DELETE FROM Entity"));
    }
}
