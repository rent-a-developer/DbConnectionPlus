...
public class DbConnectionExtensions_ExecuteReaderTests()
    ...
    )
{
    ...
    {
        ArgumentNullGuardVerifier.Verify(() => this.MockDbConnection.ExecuteReader("SELECT * FROM Entity"));

        ArgumentNullGuardVerifier.Verify(() => this.MockDbConnection.ExecuteReaderAsync("SELECT * FROM Entity"));
    }
}
