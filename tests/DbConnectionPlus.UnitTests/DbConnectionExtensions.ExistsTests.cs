...
public class DbConnectionExtensions_ExistsTests()
    ...
    )
{
    ...
    {
        ArgumentNullGuardVerifier.Verify(() => this.MockDbConnection.Exists("SELECT 1"));

        ArgumentNullGuardVerifier.Verify(() => this.MockDbConnection.ExistsAsync("SELECT 1"));
    }
}
