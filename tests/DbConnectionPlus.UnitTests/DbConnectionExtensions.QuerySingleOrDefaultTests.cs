...
{
    public DbConnectionExtensions_QuerySingleOrDefaultTests()
        : base(
    ...
    {
        ArgumentNullGuardVerifier.Verify(() => this.MockDbConnection.QuerySingleOrDefault("SELECT * FROM Entity"));

        ArgumentNullGuardVerifier.Verify(() => this.MockDbConnection.QuerySingleOrDefaultAsync("SELECT * FROM Entity"));
    }
}
