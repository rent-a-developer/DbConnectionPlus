...
{
    public DbConnectionExtensions_QuerySingleTests()
        : base(
    ...
    {
        ArgumentNullGuardVerifier.Verify(() => this.MockDbConnection.QuerySingle("SELECT * FROM Entity"));

        ArgumentNullGuardVerifier.Verify(() => this.MockDbConnection.QuerySingleAsync("SELECT * FROM Entity"));
    }
}
