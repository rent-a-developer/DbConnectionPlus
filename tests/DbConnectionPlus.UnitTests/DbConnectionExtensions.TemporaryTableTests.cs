...
#pragma warning restore RCS1163 // Unused parameter
        ...
        TemporaryTable(entityIds).Name.Should().StartWith("EntityIds_");

        TemporaryTable(GetEntityIds()).Name.Should().StartWith("EntityIds_");

        TemporaryTable(GetEntityIdsByCategory("Shoes")).Name.Should().StartWith("EntityIdsByCategoryShoes_");

        TemporaryTable(this.testEntityIds).Name.Should().StartWith("TestEntityIds_");

        TemporaryTable(Get()).Name.Should().StartWith("Values_");
    }
    ...
    public void TemporaryTable_ShouldReturnInterpolatedTemporaryTable()
    {
        ...
        temporaryTable1.Values.Should().BeSameAs(entityIds);

        temporaryTable1.ValuesType.Should().Be(typeof(long));

        temporaryTable1.Name.Should().StartWith("EntityIds_");

        var entities = Generate.Multiple<Entity>();

        var temporaryTable2 = TemporaryTable(entities);

        temporaryTable2.Values.Should().BeSameAs(entities);

        temporaryTable2.ValuesType.Should().Be(typeof(Entity));

        temporaryTable2.Name.Should().StartWith("Entities_");
    }
    ...
    public void TemporaryTable_ShouldTruncateInferredTableName()
    {
        ...
        TemporaryTable(longname_1234567890_1234567890_1234567890_1234567890_1234567890_1234567890)
            .Name.Should()
            .HaveLength(60)
