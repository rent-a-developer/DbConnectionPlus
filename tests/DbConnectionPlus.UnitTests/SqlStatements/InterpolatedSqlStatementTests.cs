...
public class InterpolatedSqlStatementTests : UnitTestsBase
{
    ...
    public void AppendFormatted_InterpolatedParameter_ShouldStoreParameter()
    {
        ...
        statement.Fragments.Should().HaveCount(2);

        statement.Fragments[0].Should().Be(new Literal("SELECT "));
    ...
    public void AppendFormatted_InterpolatedParameter_ShouldSupportComplexExpressions()
    {
        ...
        statement.Fragments.Should().HaveCount(4);
        ...
        statement
            ...
            .Should()
            .BeEquivalentTo(
                new InterpolatedParameter("EntityIdsWhereaa5ToArray0", entityIds.Where(a => a > 5).ToArray()[0])
            );
    }
    ...
    public void AppendFormatted_InterpolatedTemporaryTables_ShouldStoreTemporaryTables()
    {
        ...
        statement.TemporaryTables.Should().HaveCount(2);

        var table1 = statement.TemporaryTables[0];

        table1.Name.Should().StartWith("EntityIds_");

        table1.Values.Should().BeEquivalentTo(entityIds);
        ...
        table2.Values.Should().BeEquivalentTo(entities);

        table2.ValuesType.Should().Be(typeof(Entity));

        statement.Fragments.Should().HaveCount(4);

        statement.Fragments[0].Should().Be(new Literal("SELECT Value FROM "));
    ...
    public void AppendFormatted_MultipleInterpolatedParameters_ShouldStoreParameters()
    {
        ...
        statement.Fragments.Should().HaveCount(6);
    ...
    public void AppendFormatted_ShouldFormatAndStoreLiteral()
    {
        ...
        statement.Fragments[1].Should().Be(new Literal("    123.45"));

        statement.Fragments[2].Should().Be(new Literal(", "));

        statement.Fragments[3].Should().Be(new Literal("123.45    "));
    }
...
#pragma warning restore RCS1214 // Unnecessary interpolated string
        ...
        statement.Fragments.Should().HaveCount(1);

        statement.Fragments[0].Should().Be(new Literal("SELECT 1"));
    }
    ...
    public void Constructor_Code_Parameters_ShouldStoreParameters()
    {
        ...
        statement.Fragments[0].Should().Be(new Literal("SELECT @Parameter1, @Parameter2, @Parameter3"));

        statement.Fragments[1].Should().Be(new Parameter("Parameter1", value1));

        statement.Fragments[2].Should().Be(new Parameter("Parameter2", value2));

        statement.Fragments[3].Should().Be(new Parameter("Parameter3", value3));
    }
    ...
    public void Constructor_LiteralLength_FormattedCount_ShouldInitializeInstance()
    {
        ...
        statement.Fragments.Should().BeEmpty();
    }
    ...
    public void Fragments_ShouldGetFragments()
    {
        ...
        statement.Fragments.Should().HaveCount(10);
    ...
    public void FromString_EmptyString_ShouldCreateEmptyStatement()
    {
        ...
        statement.Fragments[0].Should().Be(new Literal(string.Empty));
    }
...
#pragma warning restore RCS1124 // Inline local variable
    ...
    public void TemporaryTables_ShouldGetInterpolatedTemporaryTables()
    {
        ...
        var table1 = statement.TemporaryTables[0].Should().BeOfType<InterpolatedTemporaryTable>().Subject;
    ...
    public void ToString_ShouldReturnStringRepresentationOfStatement()
    {
        ...
        itemsTable.Name.Should().StartWith("Items_");

        itemsTable.Values.Should().Be(items);

        itemsTable.ValuesType.Should().Be(typeof(Item));

        var idsTable = temporaryTables[1];

        idsTable.Name.Should().StartWith("Ids_");

        idsTable.Values.Should().Be(ids);

        idsTable.ValuesType.Should().Be(typeof(int));
